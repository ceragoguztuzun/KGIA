...
def build_entity_mapping(files, relation_filter=None):
    entity_set = set()
        ...
        with open(file_path, 'r') as f:
            for line in f:
                ...
                if relation_filter is None or relation == relation_filter:
                    entity_set.update([head, tail])
    entity_to_int = {entity: i for i, entity in enumerate(entity_set)}
    return entity_to_int

def load_custom_triplets(file_path, entity_to_int, relation_filter=None):
    triplets = []
    with open(file_path, 'r') as f:
        for line in f:
            head, relation, tail = line.strip().split('\t')
            if relation_filter is None or relation == relation_filter:
                if head in entity_to_int and tail in entity_to_int:  # Ensure both entities are in the mapping
                    triplets.append([entity_to_int[head], entity_to_int[tail]])
...
def generate_false_edges(num_nodes, adj_matrix, num_false_edges):
    false_edges = []
    existing_edges = set(zip(*adj_matrix.nonzero()))
...
def load_custom_data(args, logger):
    path = args.datapath
    fp_root = 'GPKG/transductive/'
    files = [os.path.join(path, f) for f in [fp_root+'train.txt', fp_root+'valid.txt', fp_root+'test.txt']]

    # Build the entity mapping based on the 'TREAT' relation in the training set
    entity_to_int = build_entity_mapping([files[0]], relation_filter='TREAT')
    num_nodes = len(entity_to_int)

    # Now load the edges using the updated mapping and filtering mechanism
    train_edges = load_custom_triplets(files[0], entity_to_int, relation_filter='TREAT')
    valid_edges = load_custom_triplets(files[1], entity_to_int)
    test_edges = load_custom_triplets(files[2], entity_to_int)

    #entity_to_int = build_entity_mapping(*files)

    #train_edges = load_custom_triplets(files[0], entity_to_int, drugs_only=True)
    #valid_edges = load_custom_triplets(files[1], entity_to_int)
    #test_edges = load_custom_triplets(files[2], entity_to_int)

    adj_train = create_adjacency_matrix(train_edges, num_nodes)
    adj_label = adj_train + adj_train.T  # Assuming undirected graph for adjacency label

    # Generate node features (using degree as a simple feature)
    degrees = adj_train.sum(axis=0).A1
    features = torch.eye(int(degrees.max()) + 1)[degrees.astype(int)] # One-hot encoding of degrees  # Convert to int

    assert adj_train.shape == (num_nodes, num_nodes), "+Adjacency matrix dimension mismatch."
    assert features.shape[0] == num_nodes, "+Feature matrix dimension mismatch."
    print('num nodes:', num_nodes)

    dim_feat = features.shape[1]

    # Generate false edges for validation and testing
    val_edges_false = generate_false_edges(num_nodes, adj_train, len(valid_edges))
    test_edges_false = generate_false_edges(num_nodes, adj_train, len(test_edges))

    # Define train_pairs (for models that need explicit positive examples)
    train_pairs = train_edges  # Simplified assumption; adjust as needed

    return adj_label, features, dim_feat, adj_train, train_pairs, valid_edges, val_edges_false, test_edges, test_edges_false, entity_to_int
