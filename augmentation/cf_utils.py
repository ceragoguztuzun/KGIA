...
def load_t_files(args, T_file, logger, adj_train, entity_to_int, features):
    T_f = get_t(adj_train, args.t, args.k, args.selfloopT)

    assert adj_train.shape[0] == features.shape[0], "mismatchu"

    T_cf, adj_cf, edges_cf_t0, edges_cf_t1 = get_CF(adj_train, features, T_f, args.dist, args.gamma, args.n_workers, entity_to_int)
...
def get_CF(adj, node_embs, T_f, dist='euclidean', thresh=50, n_workers=20, entity_to_int=None):
    ...
    assert node_embs.shape[0] == adj.shape[0], "Mismatch between number of nodes in embeddings and adjacency matrix"
    assert simi_mat.shape == (adj.shape[0], adj.shape[0]), "simi_mat shape mismatch"
    assert node_nns.shape[0] == adj.shape[0], "node_nns and adj shape mismatch"
    
    assert all(0 <= a < adj.shape[0] and 0 <= b < adj.shape[0] for a, b in node_pairs), "Global node pair check failed."
    ...
    for batch in batches:
        assert all(0 <= a < adj.shape[0] and 0 <= b < adj.shape[0] for a, b in batch), "Batch contains out-of-bounds indices."

    results = pool.map(get_CF_single, [(adj, simi_mat, node_nns, T_f, thresh, np_batch, True, entity_to_int) for np_batch in batches])
...
def get_CF_single(params):
    ...
    adj, simi_mat, node_nns, T_f, thresh, node_pairs, verbose, entity_to_int = params

    assert all(0 <= a < adj.shape[0] and 0 <= b < adj.shape[0] for a, b in node_pairs), "1) Global node pair check failed."

    # Invert the mapping
    int_to_entity = {v: k for k, v in entity_to_int.items()}

    # Save the mapping dictionary
    with open('int_to_entity.pkl', 'wb') as f:
    ...
    assert all(0 <= a < adj.shape[0] and 0 <= b < adj.shape[0] for a, b in node_pairs), "2) Global node pair check failed."

    for a, b in tqdm(node_pairs, desc='Processing node pairs'):
        if not (0 <= a < node_nns.shape[0] and 0 <= b < node_nns.shape[0]):
            print(f'Invalid pair encountered! {a}, {b} with node_nns shape: {node_nns.shape[0]}')
            continue
        ...
        while i < len(nns_a)-1 and j < len(nns_b)-1:
            ...
            if T_f[nns_a[i], nns_b[j]] != T_f[a, b]:
                ...
                if T_cf[a, b] == 0:
                    ...
                    #if nns_a[i] in int_to_entity and nns_b[j] in int_to_entity:
                    #    original_entity_a = int_to_entity[nns_a[i]]
                    #    original_entity_b = int_to_entity[nns_b[j]]
                    #    print(f"{nns_a[i]} is '{original_entity_a}' paired w {nns_b[j]} is '{original_entity_b}'.")
                    #else:
                    #    print(f"No original entity found for integer {nns_a[i]} or {nns_b[j]}.")
                    #print('->',[nns_a[i], nns_b[j]]) ##These edges represent hypothetical connections where, if they were to exist, they would introduce or affirm a positive treatment effect.
